...
import { applyCustomTheme } from './ww-config';
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const gridTheme = computed(() => 
      applyCustomTheme(props.content?.themeParams || {
        accentColor: "#0086F4",
    ...
    const gridThemeClass = computed(() => 
      props.content?.themeParams?.theme || 'ag-theme-quartz'
    );
    ...
    const initializeGrid = async () => {
      ...
      try {
        const theme = props.content?.themeParams?.theme || 'quartz';
        ...
        if (!gridApi) {
          new agGrid.Grid(agGridElement.value, gridOptions);
        }
      } catch (error) {
